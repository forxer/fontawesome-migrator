...
class FontAwesomeMappingGenerator
{
    ...
    {
        if (!isset($this->criticalMappings[$versionKey])) {
            $this->output("⚠️  Pas de mappings critiques définis pour {$versionKey}");
            return;
        }

        $this->output("🔍 Validation des mappings critiques:");
        
        foreach ($this->criticalMappings[$versionKey] as $old => $expected) {
            if (isset($mappings[$old])) {
                if ($mappings[$old] === $expected) {
                    $this->output("  ✅ {$old} → {$expected}");
                } else {
                    $this->output("  ❌ {$old} → {$mappings[$old]} (attendu: {$expected})");
                }
            } else {
                $this->output("  ⚠️  {$old} manquant dans les mappings");
            }
        }
    }
