...
- **BUG CRITIQUE WARNINGS (Août 2025)**: Système de warnings défectueux après implémentation migration progressive. Les warnings doivent guider l'utilisateur pour ses vérifications mais actuellement soit absents soit tous présents. Problème dans extractWarnings() et/ou logique getSpecificWarnings() des mappers FA5→6. Urgence haute car impact utilisabilité majeur - les warnings sont essentiels pour la validation des migrations.
- **Août 2025 - Optimisation metadata.json v2.0.1**: Structure allégée - suppression `original_file`, `relative_path`, `content` redondants. Tous chemins (`file`, `backup_path`) relatifs à l'application incluant `migrations_path` configuré. Gain significatif en taille et cohérence des métadonnées.
...
- **Août 2025 - Migration progressive automatique v2.1.0 OPÉRATIONNELLE**: Fix critique BaseVersionMapper::findIconMapping() - détection automatique structure mappings FA4→5 vs FA5→6. Migration 4→5→6→7 complète validée : 1195 icônes migrées automatiquement en une commande. Système intelligent détecte toutes versions présentes et exécute séquence optimale.
- **Architecture de mapping unifiée v2.1.0**: Structure FA4→5 (clés sans préfixe) vs FA5→6/6→7 (avec préfixe fa-) automatiquement détectée et gérée. Plus de régressions possibles entre versions.
- **Test complet migration progressive validé**: Séquence 4→5→6→7 parfaitement opérationnelle - fa-cog→fa-gear, fa-home→fa-house, fa-save→fa-floppy-disk, outlined icons correctement gérés
- **Août 2025 - Gestion intelligente versionnement v2.1.0**: DirectoryHelper génère .gitignore adaptatif - metadata.json TOUJOURS versionné, backups ignorés en dry-run mais versionnés en mode réel pour traçabilité. Workflow collaboratif : partage migrations entre développeurs via Git.
- **IMPORTANT - Logique .gitignore inversée clarifiée**: En dry-run on ignore les backups (test sans impact), en mode réel on les versionne (traçabilité complète). Protection "belt and suspenders" même si backups pas générés en dry-run.